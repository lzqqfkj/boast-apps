...
class Employee < ActiveRecord::Base
  ...
  validates_length_of       :login,:minimum => 4,:message => "用户名长度须为4到20位字母或数字!" #, :if => :login.nil?
  validates_uniqueness_of   :login,:case_sensitive => false,:message =>"该用户名已存在!"   #:uniqueness 
  ...
  validates_confirmation_of :password,:message => "两次密码不一致!"     #:confirmation
...
#  attr_accessible :login, :email, :password, :password_confirmation,:nikename
